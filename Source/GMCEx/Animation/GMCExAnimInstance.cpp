...
#include "GMCExAnimInstance.h"

void UGMCExAnimInstance::ObtainComponents()
{
	if (!IsValid(GMCPawn) || !IsValid(MovementComponent))
	{
		...
		if (!GetWorld()->IsGameWorld())
		{
			...
			MovementComponent = Cast<UGMCExMovementComponent>(GMCPawn->GetMovementComponent());

			if (!MovementComponent)
			{
				MovementComponent = NewObject<UGMCExMovementComponent>();
			}
		...
		{
			MovementComponent = Cast<UGMCExMovementComponent>(GMCPawn->GetComponentByClass<UGMCE_OrganicMovementCmp>());
		}
	}	
}


void UGMCExAnimInstance::NativeInitializeAnimation()
{
...
void UGMCExAnimInstance::NativeBeginPlay()
{
...
void UGMCExAnimInstance::NativeUpdateAnimation(float DeltaSeconds)
{
